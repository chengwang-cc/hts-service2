...
async function main() {
  ...
  const queries = [
    'ossm complete diy kit includes printed parts wired',
    'high density polyethylene hdpe plastic block',
    'bearing set',
    'wood art',
    'wood colour samples',
    '100% acrylic keychains with metal clasps',
  ];
  ...
  for (const q of queries) {
    ...
    if (withChapters.length) {
      ...
      console.log(`  No blocking. Relevant rules: ${relevant.map(r=>r.id).join(', ') || 'none'}`);
    }
    console.log();
  }
