...
export class CsvExportService {
  ...
  generateAuditPack(classification: any): Buffer {
    const records = [
      {
        'Classification Date':
          classification.confirmedAt || classification.createdAt,
   ...
   */
  generatePackingListExport(data: { packages: any[] }): Buffer {
  ...
  private getAdCvdCaseNumber(calculation: any): string {
    if (
      calculation.results?.antidumpingDuty ||
      calculation.results?.countervailingDuty
    ) {
