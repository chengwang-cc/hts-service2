...
async function main() {
  ...
  try {
    ...
    const targetRules = [
      ...
      'STUFFED_ANIMAL_INTENT',
      'HOCKEY_STICK_INTENT',
      'AI_CH19_STUFFED_PASTA',
      'CERAMIC_TILE_INTENT',
      // already known but confirm
    ...
    for (const ruleId of targetRules) {
      ...
      console.log(`\n=== ${ruleId} ===`);
      console.log(`anyOf (${(rule.pattern?.anyOf??[]).length}): ${JSON.stringify((rule.pattern?.anyOf??[]).slice(0,6))}`);
