  ...
  for (const q of QUERIES) {
    ...
    if (allowChRules.length) {
      for (const r of allowChRules)
        console.log(`  ALLOW ${r.id}: [${r.whitelist?.allowChapters?.join(',')}] pattern: ${JSON.stringify(r.pattern?.anyOf?.slice(0,5))}`);
    } else {
