...
async function main() {
  ...
  for (const q of queries) {
    ...
    if (withChapters.length) {
      ...
      console.log(`  No blocking, fired:`, fired.map(r => r.id).filter(id => !['PLATED_JEWELRY_INTENT','AI_CH10_QUINOA','AI_CH10_FONIO','AI_CH10_TRITICALE','AI_CH10_GRAIN_SORGHUM_SEED'].includes(id)).join(', ') || 'only noise rules');
    }
    console.log();
  }
