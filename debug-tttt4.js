...
async function main() {
  ...
  // Look at more failures by checking the eval queries for specific chapters
  // Let me check the full rule set for "bridal party banner"
  ...
  const queries = [
    ...
    '310x310mm glass build surface 3d printer glass bed',
    'crystal figurine',
    'motorcycle manual',
    'hdpe plastic block',
  ];
  ...
  for (const q of queries) {
    ...
    const blocking = fired.filter(r => r.whitelist?.allowChapters);
    const positiveChapters = new Set(fired.flatMap(r => r.whitelist?.allowChapters ?? []));
    console.log(`"${q}":`);
    console.log(`  Rules fired: ${fired.map(r => r.id).join(', ') || 'none'}`);
    console.log(`  Blocking chapters: ${[...positiveChapters].join(', ') || 'none'}`);
    console.log();
  }
