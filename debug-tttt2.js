...
async function main() {
  ...
  const { rows } = await client.query(`SELECT id, pattern, whitelist FROM lookup_intent_rule WHERE id IN ('STAINED_GLASS_FLAT_INTENT','TEMPERED_GLASS_SCREEN_INTENT','GLASS_ROD_LAMPWORK_INTENT','STICKER_SHEET_PAPER_INTENT','BONE_CHINA_CERAMIC_DISHWARE_INTENT','AI_CH75_NICKEL_MESH_CLOTH') ORDER BY id`);
